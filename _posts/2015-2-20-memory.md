...
I admit I've snipped the not-so-interesting bits from the output. Read [FAQ (Why is "strict overcommit" a dumb idea?)][landley-faq] if you're interested why the shared libraries are mapped as private, but it's the map of mordor that interests us. Let's follow the white rabbit:
...
*EDIT:* I've fixed the stupid mistakes with `alloca()` and `sizeof(char *) vs sizeof(char)`, thanks immibis and BonzaiThePenguin. Obviously I should have run the article through static analysis, but I didn't &mdash; lesson learned.
