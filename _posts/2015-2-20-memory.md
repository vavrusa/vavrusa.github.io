...
By *"portion"*, I mean that a size of each shared map is divided by the number of processes sharing it. Is that so? Let's see an example, we have an application that does read/write to a shared memory map.
